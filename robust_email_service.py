#!/usr/bin/env python3
"""
Robust Email Service for MediCare+ Platform
Handles network connectivity issues gracefully with multiple fallback mechanisms
"""
...
class RobustEmailService:
    def __init__(self):
        ...
        # Import HTTP email services for fallback
        self.http_email_service = None
        self._setup_http_email_service()
        
        if not self.email_enabled:
            print("⚠️ SMTP Email service disabled - Gmail credentials not configured")
            print("💡 Set GMAIL_EMAIL and GMAIL_APP_PASSWORD environment variables to enable email")
        else:
            print(f"✅ SMTP Email service enabled - Sender: {self.sender_email}")
            print(f"⏱️ Timeout settings: Connection={self.connection_timeout}s, Send={self.send_timeout}s, Total={self.total_timeout}s")
    
    def _setup_http_email_service(self):
        """Setup HTTP-based email service as fallback"""
        try:
            from render_http_email_service import render_http_email_service
            self.http_email_service = render_http_email_service
            print("✅ HTTP Email service ready as fallback")
        except ImportError:
            try:
                from render_builtin_email_service import render_builtin_email_service
                self.http_email_service = render_builtin_email_service
                print("✅ Builtin Email service ready as fallback")
            except ImportError:
                print("⚠️ No HTTP email service available for fallback")
                self.http_email_service = None
    ...
    def is_network_available(self) -> bool:
        ...
        try:
            ...
            # Test port connectivity with short timeout
...
<body>
    ...
    async def send_prediction_email_async(self, recipient_email: str, prediction_data: Dict[str, Any], patient_data: Dict[str, Any]) -> Dict[str, Any]:
        """Async email sending with network awareness and fallback mechanisms"""
        
        print(f"📧 Starting robust email send to {recipient_email}")
        
        # Check if SMTP email service is enabled
        if self.is_email_enabled():
            # Check network connectivity before attempting SMTP
            if self.is_network_available():
                print("✅ Network connectivity available, attempting SMTP email")
                # Try SMTP email service
                smtp_result = await self._send_via_smtp(recipient_email, prediction_data, patient_data)
                
                # If successful or it's not a network error, return the result
                if smtp_result.get("success") or not smtp_result.get("network_error"):
                    return smtp_result
                else:
                    print("⚠️ SMTP failed with network error, trying HTTP fallback")
            else:
                print("⚠️ Network connectivity not available for SMTP, trying HTTP fallback")
        else:
            print("⚠️ SMTP email service not configured, trying HTTP fallback")
        
        # Try HTTP-based email service as fallback
        if self.http_email_service:
            print("🔄 Using HTTP-based email service as fallback")
            try:
                http_result = await self.http_email_service.send_prediction_email(
                    recipient_email=recipient_email,
                    prediction_data=prediction_data,
                    patient_data=patient_data,
                    user_id="user_" + str(hash(recipient_email))
                )
                return http_result
            except Exception as e:
                print(f"❌ HTTP email service failed: {e}")
                return {
                    "success": False,
                    "message": f"Both SMTP and HTTP email services failed. Email report has been saved locally: {str(e)}",
                    "error": str(e),
                    "network_error": True
                }
        else:
            # No email service available, store locally
            print("⚠️ No email service available, storing locally")
            await self._store_email_report_locally_async(recipient_email, prediction_data, patient_data)
            return {
                "success": False,
                "message": "Email service not configured. Report saved locally and will be sent when email service is available.",
                "network_check": "skipped",
                "fallback": "local_storage"
            }
    
    async def _send_via_smtp(self, recipient_email: str, prediction_data: Dict[str, Any], patient_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"📧 Starting SMTP email send to {recipient_email} (timeout: {self.total_timeout}s)")
...
robust_email_service = RobustEmailService()